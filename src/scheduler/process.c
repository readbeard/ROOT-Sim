...
void initialize_lps(void)
{
	...
	for (i = 0; i < n_prc_tot; i++) {
		...
		lp->ECS_index = 0;
		lp->ECS_synch_table[0] = lp->gid;	// LidToGid for distributed ECS
		lp->ECS_page_list = new_list(ecs_page_node_t);
		lp->ECS_prefetch_list = new_list(ecs_page_node_t);
		lp->ECS_page_faults = 0;
		lp->ECS_current_prefetch_mode = NO_PREFETCH;
		lp->ECS_clustered_faults = 0;
		lp->ECS_scattered_faults = 0;
		lp->ECS_last_prefetch_switch = 0;
#endif
